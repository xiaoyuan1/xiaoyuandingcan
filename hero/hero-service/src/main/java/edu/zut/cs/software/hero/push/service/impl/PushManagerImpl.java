...
public class PushManagerImpl extends GenericManagerImpl<Push, Long> implements PushManager{
	...
	@Override
	public List<Push> findByCode(String postcode) {

		// 创建查询条件数据对象
		Push queryObject = new Push();
		...
		// 创建实例并查询
		Example<Push> ex = Example.of(queryObject, matcher);
		List<Push> result = dao.findAll(ex);
		return result;
	}

	@Override
	public List<Push> findByFullname(String fullname) {
		// 创建查询条件数据对象
		Push queryObject = new Push();
		queryObject.setMessage(fullname);
		// 创建匹配器，即如何使用查询条件
		ExampleMatcher matcher = ExampleMatcher.matching() // 构建对象
